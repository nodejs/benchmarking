...
jmeter_values=`cat $file|grep 'java' | awk {'print $9'}`
...
jmeter_percentage=`echo $jmeter_values|awk  'BEGIN { sum=0 ; count=0} {for(i = 3; i <= NF; i++) {sum=sum+$i; count=count+1 }} END {print sum/count}'`
...
echo "jmeter utilization $jmeter_percentage %"
rm ${1}.tmp
