...
function runBenchmarks(count) {
    var array = getArray();
    var lomutos = lomutosQS(array);
    var hoare = hoareQS(array);

    runner.runBenchmarks('es.2015.var.lomutos', function(suite) {
        runner.buildNTests(count, 'es.2015.var.lomutos', suite, lomutos);
    });
    runner.runBenchmarks('es.2015.var.hoare', function(suite) {
